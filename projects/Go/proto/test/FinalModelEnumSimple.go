...
func (fm *FinalModelEnumSimple) Get() (*EnumSimple, int, error) {
    return fm.GetDefault(EnumSimple(0))
}
...
func (fm *FinalModelEnumSimple) GetValue(value *EnumSimple) (*EnumSimple, int, error) {
    if (fm.buffer.Offset() + fm.FBEOffset() + fm.FBESize()) > fm.buffer.Size() {
        return value, 0, errors.New("model is broken")
    }

    result := EnumSimple(fbe.ReadInt32(fm.buffer.Data(), fm.buffer.Offset() + fm.FBEOffset()))
    value = &result
