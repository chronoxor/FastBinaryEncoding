...
class FinalModelOptionalBytes(buffer: com.chronoxor.fbe.Buffer, offset: Long) : com.chronoxor.fbe.FinalModel(buffer, offset)
{
    // Get the allocation size
    fun fbeAllocationSize(optional: ByteArray?): Long = 1 + (if (optional != null) value.fbeAllocationSize(optional) else 0)
    ...
    val value = com.chronoxor.fbe.FinalModelBytes(buffer, 0)
    ...
    fun get(size: com.chronoxor.fbe.Size): ByteArray?
    {
    ...
    fun set(optional: ByteArray?): Long
    {
        assert((_buffer.offset + fbeOffset + 1) <= _buffer.size) { "Model is broken!" }
        if ((_buffer.offset + fbeOffset + 1) > _buffer.size)
            return 0
