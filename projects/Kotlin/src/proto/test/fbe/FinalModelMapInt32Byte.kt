...
// Fast Binary Encoding Int32->Byte map final model class
class FinalModelMapInt32Byte(buffer: Buffer, offset: Long) : FinalModel(buffer, offset)
{
    private val _modelKey = FinalModelInt32(buffer, offset)
    private val _modelValue = FinalModelByte(buffer, offset)

    // Get the allocation size
    fun FBEAllocationSize(values: TreeMap<Int, Byte>): Long
    {
        var size: Long = 4
    ...
    fun FBEAllocationSize(values: HashMap<Int, Byte>): Long
    {
        var size: Long = 4
        for ((key, value1) in values)
        {
    ...
    fun get(values: TreeMap<Int, Byte>): Long
    {
        values.clear()
    ...
    fun get(values: HashMap<Int, Byte>): Long
    {
        values.clear()
    ...
    fun set(values: TreeMap<Int, Byte>): Long
    {
        assert(_buffer.offset + FBEOffset + 4 <= _buffer.size) { "Model is broken!" }
        if (_buffer.offset + FBEOffset + 4 > _buffer.size)
            return 0
    ...
    fun set(values: HashMap<Int, Byte>): Long
    {
        assert(_buffer.offset + FBEOffset + 4 <= _buffer.size) { "Model is broken!" }
        if (_buffer.offset + FBEOffset + 4 > _buffer.size)
            return 0
