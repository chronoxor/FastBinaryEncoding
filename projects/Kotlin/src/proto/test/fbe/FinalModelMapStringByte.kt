...
// Fast Binary Encoding String->Byte map final model class
class FinalModelMapStringByte(buffer: Buffer, offset: Long) : FinalModel(buffer, offset)
{
    private val _modelKey = FinalModelString(buffer, offset)
    private val _modelValue = FinalModelByte(buffer, offset)

    // Get the allocation size
    fun FBEAllocationSize(values: TreeMap<String, Byte>): Long {
        var size: Long = 4
    ...
    fun FBEAllocationSize(values: HashMap<String, Byte>): Long {
        var size: Long = 4
        for ((key, value1) in values) {
            size += _modelKey.FBEAllocationSize(key)
            size += _modelValue.FBEAllocationSize(value1)
    ...
    fun get(values: TreeMap<String, Byte>): Long {
        values.clear()
    ...
    fun get(values: HashMap<String, Byte>): Long {
        values.clear()
    ...
    fun set(values: TreeMap<String, Byte>): Long {
        assert(_buffer.offset + FBEOffset + 4 <= _buffer.size) { "Model is broken!" }
        if (_buffer.offset + FBEOffset + 4 > _buffer.size)
            return 0
