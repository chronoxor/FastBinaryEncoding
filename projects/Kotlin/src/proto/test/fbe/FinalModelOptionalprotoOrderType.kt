...
class FinalModelOptionalprotoOrderType(buffer: Buffer, offset: Long) : FinalModel(buffer, offset)
{
    // Get the allocation size
    fun FBEAllocationSize(optional: proto.OrderType?): Long = (1 + (if (optional != null) value.FBEAllocationSize(optional) else 0)).toLong()
    ...
    val value = proto.fbe.FinalModelOrderType(buffer, 0)
    ...
    fun get(size: Size): proto.OrderType?
    {
    ...
    fun set(optional: proto.OrderType?): Long
    {
        assert(_buffer.offset + FBEOffset + 1 <= _buffer.size) { "Model is broken!" }
        if (_buffer.offset + FBEOffset + 1 > _buffer.size)
            return 0
