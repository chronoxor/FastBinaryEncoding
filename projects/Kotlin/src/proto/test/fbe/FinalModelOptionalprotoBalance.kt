...
class FinalModelOptionalprotoBalance(buffer: Buffer, offset: Long) : FinalModel(buffer, offset)
{
    ...
    fun fbeAllocationSize(optional: proto.Balance?): Long = (1 + (if (optional != null) value.fbeAllocationSize(optional) else 0)).toLong()
    ...
    val value = proto.fbe.FinalModelBalance(buffer, 0)
    ...
    fun get(size: Size): proto.Balance?
    {
    ...
    fun set(optional: proto.Balance?): Long
    {
        assert(_buffer.offset + fbeOffset + 1 <= _buffer.size) { "Model is broken!" }
        if (_buffer.offset + fbeOffset + 1 > _buffer.size)
            return 0
