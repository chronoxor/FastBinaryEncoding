...
// Fast Binary Encoding Int32->OptionalBytes map final model class
class FinalModelMapInt32OptionalBytes(buffer: Buffer, offset: Long) : FinalModel(buffer, offset)
{
    private val _modelKey = FinalModelInt32(buffer, offset)
    private val _modelValue = FinalModelOptionalBytes(buffer, offset)

    // Get the allocation size
    fun FBEAllocationSize(values: TreeMap<Int, ByteArray?>): Long {
        var size: Long = 4
    ...
    fun FBEAllocationSize(values: HashMap<Int, ByteArray?>): Long {
        var size: Long = 4
        for ((key, value1) in values) {
            size += _modelKey.FBEAllocationSize(key)
            size += _modelValue.FBEAllocationSize(value1)
    ...
    fun get(values: TreeMap<Int, ByteArray?>): Long {
        values.clear()
    ...
    fun get(values: HashMap<Int, ByteArray?>): Long {
        values.clear()
    ...
    fun set(values: TreeMap<Int, ByteArray?>): Long {
        assert(_buffer.offset + FBEOffset + 4 <= _buffer.size) { "Model is broken!" }
        if (_buffer.offset + FBEOffset + 4 > _buffer.size)
            return 0
    ...
    fun set(values: HashMap<Int, ByteArray?>): Long {
        assert(_buffer.offset + FBEOffset + 4 <= _buffer.size) { "Model is broken!" }
        if (_buffer.offset + FBEOffset + 4 > _buffer.size)
            return 0
