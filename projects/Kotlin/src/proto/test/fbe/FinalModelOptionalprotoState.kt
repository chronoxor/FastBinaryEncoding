...
class FinalModelOptionalprotoState(buffer: Buffer, offset: Long) : FinalModel(buffer, offset)
{
    ...
    fun fbeAllocationSize(optional: proto.State?): Long = (1 + (if (optional != null) value.fbeAllocationSize(optional) else 0)).toLong()
    ...
    val value = proto.fbe.FinalModelState(buffer, 0)
    ...
    fun set(optional: proto.State?): Long
    {
        assert(_buffer.offset + fbeOffset + 1 <= _buffer.size) { "Model is broken!" }
        if (_buffer.offset + fbeOffset + 1 > _buffer.size)
            return 0
