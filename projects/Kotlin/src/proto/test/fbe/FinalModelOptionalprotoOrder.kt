...
class FinalModelOptionalprotoOrder(buffer: Buffer, offset: Long) : FinalModel(buffer, offset)
{
    ...
    fun FBEAllocationSize(optional: proto.Order?): Long = (1 + (if (optional != null) value.FBEAllocationSize(optional) else 0)).toLong()
    ...
    val value = proto.fbe.FinalModelOrder(buffer, 0)
    ...
    fun set(optional: proto.Order?): Long
    {
        assert(_buffer.offset + FBEOffset + 1 <= _buffer.size) { "Model is broken!" }
        if (_buffer.offset + FBEOffset + 1 > _buffer.size)
            return 0
