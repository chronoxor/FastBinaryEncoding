...
class FieldModelMapStringFlagsSimple(buffer: Buffer, offset: Long) : FieldModel(buffer, offset)
{
    ...
    // Map index operator
    fun getItem(index: Long): Pair<FieldModelString, FieldModelFlagsSimple> {
    ...
    fun resize(size: Long): Pair<FieldModelString, FieldModelFlagsSimple> {
    ...
    fun get(values: TreeMap<String, FlagsSimple>) {
        values.clear()
    ...
    fun set(values: TreeMap<String, FlagsSimple>) {
        assert(_buffer.offset + FBEOffset + FBESize <= _buffer.size) { "Model is broken!" }
        if (_buffer.offset + FBEOffset + FBESize > _buffer.size)
            return
    ...
    fun set(values: HashMap<String, FlagsSimple>) {
        assert(_buffer.offset + FBEOffset + FBESize <= _buffer.size) { "Model is broken!" }
        if (_buffer.offset + FBEOffset + FBESize > _buffer.size)
            return
