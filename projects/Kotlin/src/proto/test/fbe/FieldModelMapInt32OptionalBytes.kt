...
class FieldModelMapInt32OptionalBytes(buffer: Buffer, offset: Long) : FieldModel(buffer, offset)
{
    ...
    // Map index operator
    fun getItem(index: Long): Pair<FieldModelInt32, FieldModelOptionalBytes> {
    ...
    fun resize(size: Long): Pair<FieldModelInt32, FieldModelOptionalBytes> {
    ...
    fun get(values: TreeMap<Int, ByteArray?>) {
        values.clear()
    ...
    operator fun get(values: HashMap<Int, ByteArray?>) {
        values.clear()
    ...
    fun set(values: TreeMap<Int, ByteArray?>) {
        assert(_buffer.offset + FBEOffset + FBESize <= _buffer.size) { "Model is broken!" }
        if (_buffer.offset + FBEOffset + FBESize > _buffer.size)
            return
    ...
    fun set(values: HashMap<Int, ByteArray?>) {
        assert(_buffer.offset + FBEOffset + FBESize <= _buffer.size) { "Model is broken!" }
        if (_buffer.offset + FBEOffset + FBESize > _buffer.size)
            return
