...
class FinalModelMapStringOptionalEnumSimple: FinalModel {
    ...
    private var _modelKey: ChronoxorWowFbe.FinalModelString
    ...
    init(buffer: Buffer, offset: Int) {
        ...
        _modelKey = ChronoxorWowFbe.FinalModelString(buffer: buffer, offset: offset)
        _modelValue = FinalModelOptionalEnumSimple(buffer: buffer, offset: offset)
    }

    // Get the allocation size
    func fbeAllocationSize(value values: Dictionary<String, ChronoxorWowTest.EnumSimple?>) -> Int {
        var size: Int = 4
    ...
    public func get(values: inout Dictionary<String, ChronoxorWowTest.EnumSimple?>) -> Int {
        values.removeAll()
    ...
    public func set(value values: Dictionary<String, ChronoxorWowTest.EnumSimple?>) throws -> Int {
        if _buffer.offset + fbeOffset + 4 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
