...
class FinalModelVectorOptionalFlagsSimple: FinalModel {
    var _buffer: Buffer = Buffer()
    var _offset: Int = 0

    private var _model: FinalModelOptionalFlagsSimple

    init(buffer: Buffer, offset: Int) {
        _buffer = buffer
        _offset = offset

        _model = FinalModelOptionalFlagsSimple(buffer: buffer, offset: offset)
    }

    // Get the allocation size
    func fbeAllocationSize(value values: Array<ChronoxorWowTest.FlagsSimple?>) -> Int {
        var size: Int = 4
    ...
    public func get(values: inout Array<ChronoxorWowTest.FlagsSimple?>) -> Int {
        values.removeAll()
    ...
    public func set(value values: Array<ChronoxorWowTest.FlagsSimple?>) throws -> Int {
        if _buffer.offset + fbeOffset + 4 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
