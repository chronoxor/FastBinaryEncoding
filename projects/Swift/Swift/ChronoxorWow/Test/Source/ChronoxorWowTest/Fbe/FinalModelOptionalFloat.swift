...
class FinalModelOptionalFloat: FinalModel {

    var _buffer: Buffer
    var _offset: Int

    // Base field model value
    let value: ChronoxorWowFbe.FinalModelFloat
    ...
    required init() {
        ...
        value = ChronoxorWowFbe.FinalModelFloat(buffer: buffer, offset: offset)
    }
    ...
    func fbeAllocationSize(value optional: Float?) -> Int {
    ...
    public func set(value optional: Float?) throws -> Int {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
