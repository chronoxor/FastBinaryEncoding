...
class FinalModelOptionalInt8: FinalModel {

    var _buffer: Buffer
    var _offset: Int

    // Base field model value
    let value: ChronoxorWowFbe.FinalModelInt8
    ...
    required init() {
        ...
        value = ChronoxorWowFbe.FinalModelInt8(buffer: buffer, offset: offset)
    }
    ...
    public func get(size: inout Size) -> Int8? {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
    ...
    public func set(value optional: Int8?) throws -> Int {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
