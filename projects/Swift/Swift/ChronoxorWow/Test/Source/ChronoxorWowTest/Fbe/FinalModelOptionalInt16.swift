...
class FinalModelOptionalInt16: FinalModel {

    var _buffer: Buffer
    var _offset: Int

    // Base field model value
    let value: ChronoxorWowFbe.FinalModelInt16
    ...
    required init() {
        ...
        value = ChronoxorWowFbe.FinalModelInt16(buffer: buffer, offset: offset)
    }
    ...
    func fbeAllocationSize(value optional: Int16?) -> Int {
    ...
    public func get(size: inout Size) -> Int16? {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
    ...
    public func set(value optional: Int16?) throws -> Int {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
