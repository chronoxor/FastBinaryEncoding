...
// Fast Binary Encoding UInt8 vector final model
class FinalModelVectorUInt8: FinalModel {
    var _buffer: Buffer = Buffer()
    var _offset: Int = 0

    private var _model: ChronoxorWowFbe.FinalModelUInt8

    init(buffer: Buffer, offset: Int) {
        _buffer = buffer
        _offset = offset

        _model = ChronoxorWowFbe.FinalModelUInt8(buffer: buffer, offset: offset)
    }
