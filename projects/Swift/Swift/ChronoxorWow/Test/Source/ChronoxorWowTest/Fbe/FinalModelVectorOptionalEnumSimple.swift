...
class FinalModelVectorOptionalEnumSimple: FinalModel {
    var _buffer: Buffer = Buffer()
    var _offset: Int = 0

    private var _model: FinalModelOptionalEnumSimple

    init(buffer: Buffer, offset: Int) {
        _buffer = buffer
        _offset = offset

        _model = FinalModelOptionalEnumSimple(buffer: buffer, offset: offset)
    }

    // Get the allocation size
    func fbeAllocationSize(value values: Array<ChronoxorWowTest.EnumSimple?>) -> Int {
        var size: Int = 4
    ...
    public func get(values: inout Array<ChronoxorWowTest.EnumSimple?>) -> Int {
        values.removeAll()
    ...
    public func set(value values: Array<ChronoxorWowTest.EnumSimple?>) throws -> Int {
        if _buffer.offset + fbeOffset + 4 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
