...
class FinalModelOptionalWChar: FinalModel {

    var _buffer: Buffer
    var _offset: Int

    // Base field model value
    let value: ChronoxorWowFbe.FinalModelWChar
    ...
    required init() {
        ...
        value = ChronoxorWowFbe.FinalModelWChar(buffer: buffer, offset: offset)
    }
    ...
    public func set(value optional: Character?) throws -> Int {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
