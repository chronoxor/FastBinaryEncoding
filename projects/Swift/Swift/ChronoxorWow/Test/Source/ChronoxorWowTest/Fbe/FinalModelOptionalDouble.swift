...
class FinalModelOptionalDouble: FinalModel {

    var _buffer: Buffer
    var _offset: Int

    // Base field model value
    let value: ChronoxorWowFbe.FinalModelDouble
    ...
    required init() {
        ...
        value = ChronoxorWowFbe.FinalModelDouble(buffer: buffer, offset: offset)
    }
    ...
    public func set(value optional: Double?) throws -> Int {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
