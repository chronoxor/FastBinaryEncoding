...
class FinalModelOptionalUInt64: FinalModel {

    var _buffer: Buffer
    var _offset: Int

    // Base field model value
    let value: ChronoxorWowFbe.FinalModelUInt64
    ...
    required init() {
        ...
        value = ChronoxorWowFbe.FinalModelUInt64(buffer: buffer, offset: offset)
    }
    ...
    func fbeAllocationSize(value optional: UInt64?) -> Int {
    ...
    public func get(size: inout Size) -> UInt64? {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
    ...
    public func set(value optional: UInt64?) throws -> Int {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
