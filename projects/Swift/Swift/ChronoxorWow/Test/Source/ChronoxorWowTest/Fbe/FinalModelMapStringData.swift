...
// Fast Binary Encoding String->Data map final model
class FinalModelMapStringData: FinalModel {
    var _buffer: Buffer = Buffer()
    var _offset: Int = 0

    private var _modelKey: ChronoxorWowFbe.FinalModelString
    private var _modelValue: ChronoxorWowFbe.FinalModelData
    ...
    init(buffer: Buffer, offset: Int) {
        ...
        _modelValue = ChronoxorWowFbe.FinalModelData(buffer: buffer, offset: offset)
    }

    // Get the allocation size
    func fbeAllocationSize(value values: Dictionary<String, Data>) -> Int {
        var size: Int = 4
    ...
    public func get(values: inout Dictionary<String, Data>) -> Int {
        values.removeAll()
    ...
    public func set(value values: Dictionary<String, Data>) throws -> Int {
        if _buffer.offset + fbeOffset + 4 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
