...
class FinalModelOptionalDecimal: FinalModel {

    var _buffer: Buffer
    var _offset: Int

    // Base field model value
    let value: ChronoxorWowFbe.FinalModelDecimal
    ...
    required init() {
        ...
        value = ChronoxorWowFbe.FinalModelDecimal(buffer: buffer, offset: offset)
    }
    ...
    public func set(value optional: Decimal?) throws -> Int {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
