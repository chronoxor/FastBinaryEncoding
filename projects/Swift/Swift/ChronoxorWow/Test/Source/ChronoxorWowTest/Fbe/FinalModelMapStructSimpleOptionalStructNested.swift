...
class FinalModelMapStructSimpleOptionalStructNested: FinalModel {
    var _buffer: Buffer = Buffer()
    var _offset: Int = 0

    private var _modelKey: FinalModelStructSimple
    private var _modelValue: FinalModelOptionalStructNested

    init(buffer: Buffer, offset: Int) {
        _buffer = buffer
        _offset = offset

        _modelKey = FinalModelStructSimple(buffer: buffer, offset: offset)
        _modelValue = FinalModelOptionalStructNested(buffer: buffer, offset: offset)
    }

    // Get the allocation size
    func fbeAllocationSize(value values: Dictionary<ChronoxorWowTest.StructSimple, ChronoxorWowTest.StructNested?>) -> Int {
        var size: Int = 4
    ...
    public func get(values: inout Dictionary<ChronoxorWowTest.StructSimple, ChronoxorWowTest.StructNested?>) -> Int {
        values.removeAll()
    ...
    public func set(value values: Dictionary<ChronoxorWowTest.StructSimple, ChronoxorWowTest.StructNested?>) throws -> Int {
        if _buffer.offset + fbeOffset + 4 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
