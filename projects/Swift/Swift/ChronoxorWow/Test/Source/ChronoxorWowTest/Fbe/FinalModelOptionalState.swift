...
// Fast Binary Encoding optional State final model
class FinalModelOptionalState: FinalModel {

    var _buffer: Buffer
    var _offset: Int

    // Base field model value
    let value: ChronoxorWowProto.FinalModelState
    ...
    required init() {
        ...
        value = ChronoxorWowProto.FinalModelState(buffer: buffer, offset: offset)
    }
    ...
    func fbeAllocationSize(value optional: ChronoxorWowProto.State?) -> Int {
    ...
    public func get(size: inout Size) -> ChronoxorWowProto.State? {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
    ...
    public func set(value optional: ChronoxorWowProto.State?) throws -> Int {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
