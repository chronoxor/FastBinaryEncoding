...
public class StructBytesModel: Model {
    public let model: FieldModelStructBytes

    public override init(buffer: Buffer = Buffer()) {
        model = FieldModelStructBytes(buffer: buffer, offset: 4)
    ...
    static let fbeTypeConst: Int = FieldModelStructBytes.fbeTypeConst
    ...
    public func serialize(value: StructBytes) throws -> Int {
    ...
    public func deserialize() -> StructBytes { var value = StructBytes(); _ = deserialize(value: &value); return value }
    public func deserialize(value: inout StructBytes) -> Int {
        var valueRef = value

        if buffer.offset + model.fbeOffset - 4 > buffer.size {
            valueRef = StructBytes()
            return 0
        }
