...
class FinalModelOptionalState: FinalModel {
    ...
    let value: proto.FinalModelState
    ...
    required init() {
        ...
        value = proto.FinalModelState(buffer: buffer, offset: offset)
    }
    ...
    func fbeAllocationSize(value optional: proto.State?) -> Int {
    ...
    public func get(size: inout Size) -> proto.State? {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
    ...
    public func set(value optional: proto.State?) throws -> Int {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
