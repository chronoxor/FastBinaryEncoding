...
public class StructSetFinalModel: Model {
    private let _model: FinalModelStructSet

    public override init(buffer: Buffer = Buffer()) {
        _model = FinalModelStructSet(buffer: buffer, offset: 8)
    ...
    static let fbeTypeConst: Int = FinalModelStructSet.fbeTypeConst
    ...
    public func serialize(value: StructSet) throws -> Int {
    ...
    public func deserialize() -> StructSet { var value = StructSet(); _ = deserialize(value: &value); return value }

    public func deserialize(value: inout StructSet) -> Int {
        if (buffer.offset + _model.fbeOffset) > buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
