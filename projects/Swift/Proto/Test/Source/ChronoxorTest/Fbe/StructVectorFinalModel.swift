...
public class StructVectorFinalModel: Model {
    private let _model: FinalModelStructVector

    public override init(buffer: Buffer = Buffer()) {
        _model = FinalModelStructVector(buffer: buffer, offset: 8)
    ...
    static let fbeTypeConst: Int = FinalModelStructVector.fbeTypeConst
    ...
    public func serialize(value: StructVector) throws -> Int {
    ...
    public func deserialize() -> StructVector { var value = StructVector(); _ = deserialize(value: &value); return value }

    public func deserialize(value: inout StructVector) -> Int {
        if (buffer.offset + _model.fbeOffset) > buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
