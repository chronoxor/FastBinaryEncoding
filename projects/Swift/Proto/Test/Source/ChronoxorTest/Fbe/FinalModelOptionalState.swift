...
class FinalModelOptionalState: FinalModel {
    ...
    let value: ChronoxorProto.FinalModelState
    ...
    required init() {
        ...
        value = ChronoxorProto.FinalModelState(buffer: buffer, offset: offset)
    }
    ...
    func fbeAllocationSize(value optional: ChronoxorProto.State?) -> Int {
    ...
    public func get(size: inout Size) -> ChronoxorProto.State? {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
    ...
    public func set(value optional: ChronoxorProto.State?) throws -> Int {
        if _buffer.offset + fbeOffset + 1 > _buffer.size {
            assertionFailure("Model is broken!")
            return 0
        }
