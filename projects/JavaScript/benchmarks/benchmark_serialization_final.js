...
suite.on('start', function () {
  ...
  this.writer = new proto.AccountFinalModel(new fbe.WriteBuffer())
  this.writer.serialize(this.account)
  console.assert(this.writer.verify())

  // Deserialize the account from the FBE stream
  this.reader = new proto.AccountFinalModel(new fbe.ReadBuffer())
...
suite.add('Serialize (Final)', function () {
  // Reset FBE stream
  suite.writer.reset()
...
suite.add('Deserialize (Final)', function () {
  // Deserialize the account from the FBE stream
  suite.reader.deserialize(suite.account)
})

suite.add('Verify (Final)', function () {
  // Verify the account
  suite.writer.verify()
})
