...
namespace Tests
{
    ...
    public class SerializationFinal
    {
        ...
        public void SerializationProto()
        {
            ...
            Assert.That(writer.Buffer.Size == 152);
        ...
        [TestCase(TestName = "Serialization (Final): struct simple")]
        public void SerializationStructSimple()
        {
            ...
            Assert.That(writer.FBEType == 110);
            ...
            // Check the serialized FBE size
            Assert.That(writer.Buffer.Size == 304);

            // Deserialize the struct from the FBE stream
            var reader = new FBE.test.StructSimpleFinalModel();
            Assert.That(reader.FBEType == 110);
            reader.Attach(writer.Buffer);
            Assert.That(reader.Verify());
        ...
        [TestCase(TestName = "Serialization (Final): struct optional")]
        public void SerializationStructOptional()
        {
            ...
            Assert.That(writer.FBEType == 111);
            long serialized = writer.Serialize(struct1);
            Assert.That(serialized == writer.Buffer.Size);
            Assert.That(writer.Verify());
            writer.Next(serialized);
            ...
            // Check the serialized FBE size
            Assert.That(writer.Buffer.Size == 478);

            // Deserialize the struct from the FBE stream
            var reader = new FBE.test.StructOptionalFinalModel();
            Assert.That(reader.FBEType == 111);
            reader.Attach(writer.Buffer);
            Assert.That(reader.Verify());
            long deserialized = reader.Deserialize(out var struct2);
            Assert.That(deserialized == reader.Buffer.Size);
        ...
        [TestCase(TestName = "Serialization (Final): struct nested")]
        public void SerializationStructNested()
        {
            ...
            Assert.That(writer.FBEType == 112);
            long serialized = writer.Serialize(struct1);
            Assert.That(serialized == writer.Buffer.Size);
            Assert.That(writer.Verify());
            writer.Next(serialized);
            ...
            // Check the serialized FBE size
            Assert.That(writer.Buffer.Size == 1267);

            // Deserialize the struct from the FBE stream
            var reader = new FBE.test.StructNestedFinalModel();
            Assert.That(reader.FBEType == 112);
            reader.Attach(writer.Buffer);
            Assert.That(reader.Verify());
            long deserialized = reader.Deserialize(out var struct2);
            Assert.That(deserialized == reader.Buffer.Size);
        ...
        [TestCase(TestName = "Serialization (Final): struct bytes")]
        public void SerializationStructBytes()
        {
            ...
            Assert.That(writer.FBEType == 120);
            long serialized = writer.Serialize(struct1);
            Assert.That(serialized == writer.Buffer.Size);
            Assert.That(writer.Verify());
            writer.Next(serialized);
            ...
            // Check the serialized FBE size
            Assert.That(writer.Buffer.Size == 25);

            // Deserialize the struct from the FBE stream
            var reader = new FBE.test.StructBytesFinalModel();
            Assert.That(reader.FBEType == 120);
            reader.Attach(writer.Buffer);
            Assert.That(reader.Verify());
            long deserialized = reader.Deserialize(out var struct2);
            Assert.That(deserialized == reader.Buffer.Size);
        ...
        [TestCase(TestName = "Serialization (Final): struct array")]
        public void SerializationStructArray()
        {
            ...
            Assert.That(writer.FBEType == 125);
            long serialized = writer.Serialize(struct1);
            Assert.That(serialized == writer.Buffer.Size);
            Assert.That(writer.Verify());
            writer.Next(serialized);
            ...
            // Check the serialized FBE size
            Assert.That(writer.Buffer.Size == 954);

            // Deserialize the struct from the FBE stream
            var reader = new FBE.test.StructArrayFinalModel();
            Assert.That(reader.FBEType == 125);
            reader.Attach(writer.Buffer);
            Assert.That(reader.Verify());
            long deserialized = reader.Deserialize(out var struct2);
            Assert.That(deserialized == reader.Buffer.Size);
        ...
        [TestCase(TestName = "Serialization (Final): struct vector")]
        public void SerializationStructVector()
        {
            ...
            Assert.That(writer.FBEType == 130);
            long serialized = writer.Serialize(struct1);
            Assert.That(serialized == writer.Buffer.Size);
            Assert.That(writer.Verify());
            writer.Next(serialized);
            ...
            // Check the serialized FBE size
            Assert.That(writer.Buffer.Size == 994);

            // Deserialize the struct from the FBE stream
            var reader = new FBE.test.StructVectorFinalModel();
            Assert.That(reader.FBEType == 130);
            reader.Attach(writer.Buffer);
            Assert.That(reader.Verify());
            long deserialized = reader.Deserialize(out var struct2);
            Assert.That(deserialized == reader.Buffer.Size);
        ...
        [TestCase(TestName = "Serialization (Final): struct list")]
        public void SerializationStructList()
        {
            ...
            Assert.That(writer.FBEType == 131);
            long serialized = writer.Serialize(struct1);
            Assert.That(serialized == writer.Buffer.Size);
            Assert.That(writer.Verify());
            writer.Next(serialized);
            ...
            Assert.That(reader.FBEType == 131);
            reader.Attach(writer.Buffer);
            Assert.That(reader.Verify());
            long deserialized = reader.Deserialize(out var struct2);
            Assert.That(deserialized == reader.Buffer.Size);
        ...
        [TestCase(TestName = "Serialization (Final): struct set")]
        public void SerializationStructSet()
        {
            ...
            Assert.That(writer.FBEType == 132);
            long serialized = writer.Serialize(struct1);
            Assert.That(serialized == writer.Buffer.Size);
            Assert.That(writer.Verify());
            writer.Next(serialized);
            ...
            // Check the serialized FBE size
            Assert.That(writer.Buffer.Size == 635);

            // Deserialize the struct from the FBE stream
            var reader = new FBE.test.StructSetFinalModel();
            Assert.That(reader.FBEType == 132);
            reader.Attach(writer.Buffer);
            Assert.That(reader.Verify());
            long deserialized = reader.Deserialize(out var struct2);
            Assert.That(deserialized == reader.Buffer.Size);
        ...
        [TestCase(TestName = "Serialization (Final): struct map")]
        public void SerializationStructMap()
        {
            ...
            Assert.That(writer.FBEType == 140);
            long serialized = writer.Serialize(struct1);
            Assert.That(serialized == writer.Buffer.Size);
            Assert.That(writer.Verify());
            writer.Next(serialized);
            ...
            // Check the serialized FBE size
            Assert.That(writer.Buffer.Size == 1074);

            // Deserialize the struct from the FBE stream
            var reader = new FBE.test.StructMapFinalModel();
            Assert.That(reader.FBEType == 140);
            reader.Attach(writer.Buffer);
            Assert.That(reader.Verify());
            long deserialized = reader.Deserialize(out var struct2);
            Assert.That(deserialized == reader.Buffer.Size);
        ...
        [TestCase(TestName = "Serialization (Final): struct hash")]
        public void SerializationStructHash()
        {
            ...
            Assert.That(writer.FBEType == 141);
            long serialized = writer.Serialize(struct1);
            Assert.That(serialized == writer.Buffer.Size);
            Assert.That(writer.Verify());
            writer.Next(serialized);
            ...
            // Check the serialized FBE size
            Assert.That(writer.Buffer.Size == 1114);

            // Deserialize the struct from the FBE stream
            var reader = new FBE.test.StructHashFinalModel();
            Assert.That(reader.FBEType == 141);
            reader.Attach(writer.Buffer);
            Assert.That(reader.Verify());
            long deserialized = reader.Deserialize(out var struct2);
            Assert.That(deserialized == reader.Buffer.Size);
        ...
        [TestCase(TestName = "Serialization (Final): struct hash extended")]
        public void SerializationStructHashExtended()
        {
            ...
            Assert.That(writer.FBEType == 142);
            long serialized = writer.Serialize(struct1);
            Assert.That(serialized == writer.Buffer.Size);
            Assert.That(writer.Verify());
            writer.Next(serialized);
            ...
            // Check the serialized FBE size
            Assert.That(writer.Buffer.Size == 4979);

            // Deserialize the struct from the FBE stream
            var reader = new FBE.test.StructHashExFinalModel();
            Assert.That(reader.FBEType == 142);
            reader.Attach(writer.Buffer);
            Assert.That(reader.Verify());
            long deserialized = reader.Deserialize(out var struct2);
            Assert.That(deserialized == reader.Buffer.Size);
