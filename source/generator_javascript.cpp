...
void GeneratorJavaScript::GenerateHeader()
{
...
void GeneratorJavaScript::GenerateFBEBuffer()
{
    std::string code = R"CODE(
/**
...
void GeneratorJavaScript::GenerateSender(const std::shared_ptr<Package>& p, bool final)
{
    ...
    if (p->import)
    {
        for (const auto& import : p->import->imports)
        {
            WriteLineIndent("let result = this._" + CppCommon::StringUtils::ToLower(*import) + "Sender.send(value)");
