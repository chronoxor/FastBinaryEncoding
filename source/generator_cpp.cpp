...
void GeneratorCpp::GenerateClient(const std::shared_ptr<Package>& p, bool final)
{
    ...
    for (const auto& response : responses)
    {
        ...
        WriteLineIndent("auto id = it_by_timestamp_" + response_field + "->second;");
        WriteLineIndent("auto timestamp = std::get<0>(it_by_id_" + response_field + ");");
        WriteLineIndent("auto timeout = std::get<1>(it_by_id_" + response_field + ");");
        WriteLineIndent("if ((timestamp + (timeout * 1000000)) <= utc)");
        WriteLineIndent("{");
        Indent(1);
        WriteLineIndent("auto& promise = std::get<2>(it_by_id_" + response_field + ");");
